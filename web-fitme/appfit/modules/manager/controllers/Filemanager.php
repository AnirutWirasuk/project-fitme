...
class Filemanager extends MX_Controller {
	...
	public function jsonListdata(){
		...
		$condition = array();
		$condition['fide'] = "file_id,file_name,file_folder,file_lastedit";
		$condition['where'] = $whereData;
		$condition['orderby'] = "file_lastedit DESC";
	...
	public function create(){
		if($this->tokens->verify('crffilemanager')){
			$data = array(
				...
				'file_editby' => $this->encryption->decrypt($this->input->cookie('sysli')),
				'file_lastedit' => date('Y-m-d H:i:s'),
				'file_status' => 1
			);
	...
	public function delete($Id){
		...
		$condition = array();
		$condition['fide'] = "file_id,file_name,file_folder,file_lastedit";
		$condition['where'] = array('file_status' => 1, 'file_id' => $Id);
		$data['listdata'] = $this->filemanager->listData($condition);
